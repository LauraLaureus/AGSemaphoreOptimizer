...
for i=1:siz
    ...
    %tic;
    fitnesses = computeFitness(Pop,sim)
    %toc;
    %disp('Time for selection')
    %tic;
    seleccion = probabilistic_tourneau(fitnesses,Pop,probability_selection);
    %toc;
    %disp('Time for crossover')
    %tic;
    Pop = three_parent_crossover(seleccion,mask1,mask2,mask3);
    %toc;
    %disp('Time for mutation')
    %tic;
    Pop = mutation(p_mutation,Pop);
    %toc;
